...
**auto参数自动回调介绍：**
传统的之前的模式是创建订单后，用户手动点击支付界面上的[我已经支付]按钮去检查订单是否支付成功，如果成功则页面发送jump跳转。这是auto=no的模式。也是之前的模式。 
现在我们支持自动回调了，创建订单时指定auto=yes，则用户哪怕关闭了支付页面，只要有支付成功，我们的服务器就会调用访问jump上的网址。
