...
| auto  | 是否启用自动回调,仅支持yes或者填no | 否 | no | 
...
需要为您的地址设置商业保护，IP白名单或者创建支付订单需要检验所有权，可邮件联系我们免费设置即可。  

**auto参数自动回调介绍：**  
传统的之前的模式是创建订单后，用户手动点击支付界面上的[我已经支付]按钮去发起ajax请求检查订单是否支付成功，如果成功则页面发生jump跳转。这是auto=no的模式。也是之前的模式。   
现在我们支持自动回调了，创建订单时指定auto=yes，则用户哪怕关闭了支付页面，只要有支付成功(钱包收到USDT)，我们的服务器就会自动调用访问订单指定的jump上的网址（这不需要你做什么）。  
注意：本功能提供的自动回调并不是实时回调，我们是在创建订单后的第5分钟和第15分钟进行订单支付状态检查。这是我们后台自己去发起这两次检查的（只要你创建订单时指定auto=yes）。     
下单时auto只支持yes和no，当查询订单时auto还可能返回first和finish两个文本值。出现no表示订单没有开启自动后台回调，出现yes表示订单开启回调等待回调。出现first表示订单已经进行第一次检查支付状态（通常在订单创建后的第5分钟），出现finish表示订单已经在我们的服务器经历了2次订单支付状态检查（第5分钟和第15分钟）。这两次检查中如果订单为支付成功（status=1）则会以GET的形式自动访问下单时的jump参数URL。请自行在JUMP地址中添加订单号等信息。   
除此之外要说的是，我们服务器调用jump的URL可能会多次。所以如果是增加金额充值类，请注意回调网址作好防重准备。  
另外，我们不会在超过15分钟后进行auto自动订单支付状态检查，这是因为毫无必要，我们的订单有效期也在15分钟以内。  
