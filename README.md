# tronusdt
用于快速为您的网站或APP接入USDT钱包功能或USDT收款功能，而无需任何区块链知识，只要会发起HTTP请求，就能对接成功，整个框架的设计宗旨是让每个人，任何编程语言都仅需要5分钟即可完成所有接入。   
此框架由波场创始人孙晨宇旗下的tron.network的子建业项目，由国人Dyjasan团队长期终身维护，属于公益项目，长期免费。     
...
2:收入自号：可设置自己的USDT钱包作为收款钱包，无需提供密钥和助记词，不经过第三方账户，安全和放心。     
3:提供源码：开源项目，目前提供了PHP源码(payui.php)以及其它主流类源码都有提供，没错每种编程语言只需要1个文件，20行左右代码，即可运行这个框架或收款系统。     
4:可用性高：本框架长期可用，稳定性100%可用，长期有团队维护，截止更新日期目前有479个商家使用本框架集成USDT插件进行收款或钱包开发。    
...
7:支援支持所有灰色黑色业务，无需任何实名资料，创建的订单每隔一段时间自动删除。对于整个账户，可发邮件删除账户。保持区块链的匿名性。[点此查看](https://tronusdtemp.xyz/help.php?bottom=yes)我们的用户隐私政策协议。     

# 支持多皮肤界面
内置超过30+款可选支付页皮肤,尾部可自定义代码元素，这里给出几个示例而已，请自行到订单后台查看和设置皮肤。    
对于所有外观不满意的，也可以自己编写HTML代码自行设计页面（way=pay后返回支付所需的json数据然后自行写页面即可）。        
...
| note | 新版新增参数:支持在支付页面底部追加html代码或文字，长度不很长一般问题不大仅支持GET传入，这个也可以到订单后台设置 | 否 | 请尽快支付有问题联系客服 |   
...
+ jump参数可填写为一个网址，则必须为http开头，它将在收款成功后进行页面跳转，如果您启动auto为回调模式(async/yes)，JUMP则代表后台回调URL。也可填写为一句话或一个商品卡密，它将在收款成功后显示出来。如您不懂此参数可以直接填写-，但不能留空。URL中如果有&，可以使用|替代网址中的&。
...
（*注意：如果您发现回调地址jump为payok.php，请阅读本文档底部的《回调地址变换说明》说明，或在订单后台底部的说明也会提到该问题）    
...
****forger说明**** 
回调状态auto若为forger表示遗忘，即检查多次过，但检查结果都为未支付,后续不再检查了，哪怕后续支付了。也不再检查和更改订单状态，通常在订单有效期内（在15分钟内），除非auto=no，系统会自动检查，检查了没支付，会多次再检查，但超过15分钟那就不再检查了，哪怕你在15分钟后支付，那订单状态也不会改为成功(status=1)。    
...
这在我们的老款视频长版教程中讲解中有提到，位于视频的44分钟20秒左右。     
***如需修改订单的金额增加每次幅度，可使用您的注册邮箱和我们邮件联系。***    
...
#### 回调地址变换说明？ 
paycheck接口访问时，在auto=回调模式时(auto不等于no一般都为回调模式)，返回的jump为源回调域名后payok.php，而不是原来的回调URL，确保回调地址不被客户用户看到。但实际不影响回调URL，请到订单后台可看到真实回调网址)     

#### USDT转账手续费TRX优惠方法推荐：  
传统USDT转账是需要消耗大约10个左右的TRX，这是整个USDT决定的，其数量根据网络拥挤程度决定。     
但通过以下电报机器人，只需2个TRX，即可完成USDT转账（不看USDT转账数量），统一大约为2个TRX。     
其原理是：您向机电报机器人，发送TRX后（发送数量请看机器人上边的要求），对方将回馈您波长能量（有能量后则转U无需TRX手续费）。    
请注意：请务必使用要转U的钱包的TRX钱包向机器人页面提出的TRX地址转TRX，以换取能量，且能量基本在转TRX后3分钟必到帐。   
以前转USDT要10个trx左右。现在变为2个很划算，可以省下不少资金。    
Telegram机器人A：https://t.me/MenteTaticaBot    
Telegram机器人B：https://t.me/TRXZLY888bot     
（上述机器人任选一个即可，我通常选择第一个，要求先安装了Telegram软件才能查看机器人，后期我会出一期专门将转U手续费的视频教程）      

#### 182错误码说明：  
如果创建订单时，提示182错误码。这是因为有不法人士使用攻击，攻击手段为采用自动脚本批量创建订单，导致该地址的支付金额不断抬高。所以目前并不是所有人都能针对TJgooLjpQkZmxW1jgjscT8sCy7xaT4t8ZG生成订单，我们为这个地址设置了调用IP白名单、以及设置了生成订单需要检验密码(传入正确的namepass参数)、或者输入验证码也能创建订单成功。  
