...
| jump  | 支付成功后提示的信息或跳转的网址或回调URL，URL中的&可用|替代 | 是 | https://baidu.com | 
...
+ jump参数可填写为一个网址，则必须为http开头，它将在收款成功后进行页面跳转，如果您启动auto为回调模式(async/yes)，JUMP则代表后台回调URL。也可填写为一句话或一个商品卡密，它将在收款成功后显示出来。如您不懂此参数可以直接填写-，但不能留空。可以使用|替代网址中的&。
